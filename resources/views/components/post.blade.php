@props(['feed', 'posts', 'user', 'originalPost'])
        ...
        <div class="post rounded-lg shadow-md p-4 mb-4 border">
            ...
            <div class="post rounded-lg shadow-md p-4 mb-4 border">
                ...
                <div class="original-post">
                    <div class="flex items-center mb-2">
                        <!-- User post profile picture -->
                        <x-profile-picture :profilePicture="$item->post->user->profilePicture" :userName="$item->post->user->name" />
                        <div class="flex flex-col">
                            <h3 class="text-lg font-semibold">{{ $item->post->user->name }}</h3>
                            <p class="text-sm text-gray-600">{{ $item->post->created_at->diffForHumans() }}</p>
                        </div>
                    </div>
                    <p class="text-gray-800 font-medium text-md leading-7">{{ $item->post->content }}</p>
                    <!-- Display image if available -->
                    @if ($item->post->image_path)
                    <div class="w-full bg-gray-100 overflow-hidden">
                        <div class="h-[800px] mx-auto">
                            <img src="{{ asset('storage/' . $item->post->image_path) }}" alt="Post Image" class="w-full h-full object-cover object-top">
                        </div>
                    </div>
                    @endif
                    <!-- Display video if available -->
                    @if ($item->post->video_path)
                    <div class="w-full bg-gray-900 overflow-hidden">
                        <div class="h-[800px] mx-auto">
                            <video controls class="w-full mt-4 h-full">
                                <source src="{{ asset('storage/' . $item->post->video_path) }}" type="video/mp4">
                                Your browser does not support the video tag.
                            </video>
                        </div>
                    </div>
